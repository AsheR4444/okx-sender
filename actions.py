...
class OKXActions:
    ...
    async def collect_funds_from_subaccounts(self, token_symbol='ETH'):
        for subaccount_name in await self.okx_client.subaccount.list():
            ...
            for balance in balances.values():
                ...
                if avail_bal > 0:
                    await self.okx_client.asset.transfer(
                        token_symbol=token_symbol, amount=avail_bal, to_=AccountTypes.Funding, subAcct=subaccount_name,
    ...
    async def get_subaccounts_frozen_balances(self):
        total_frozen_bal = 0
        for subaccount_name in await self.okx_client.subaccount.list():
            balances = await self.okx_client.subaccount.asset_balances(subAcct=subaccount_name, token_symbol='SOL')
            for balance in balances.values():
